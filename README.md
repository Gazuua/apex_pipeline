...
> 📊 [**벤치마크 보고서 허브**](https://gazuua.github.io/apex_pipeline/benchmark/) · [v0.6.5 i7-14700 3-way 비교](https://gazuua.github.io/apex_pipeline/benchmark/v0.6.5.0/i7-14700-20C28T/report.html) · [v0.6.5 i5-9300H 3-way 비교](https://gazuua.github.io/apex_pipeline/benchmark/v0.6.5.0/i5-9300H-4C8T/report.html)
...
**CPU Affinity + NUMA Binding**: 워커 스레드를 물리 코어에 1:1 핀닝하여 캐시 히트율을 극대화합니다. Intel 하이브리드 아키텍처(Alder Lake+)에서는 P-Core를 우선 할당하고, 멀티소켓 서버에서는 NUMA `set_mempolicy`로 로컬 메모리 접근을 보장합니다. `num_cores = 0`이면 물리 코어 수를 자동 감지하여 HT/SMT 시블링 제외 후 최적 워커 수를 결정합니다.
...
<td>Go 데몬 + SQLite — 브랜치 상태 머신, 백로그 추적, 빌드/머지 큐 락, 웹 대시보드 4페이지 (관찰 전용)</td>
<td align="center"><b>11,000+ LOC</b> · 4 모듈 · 14 테스트 패키지</td>
</tr>
<tr>
...
<td align="center"><b>6 강제 게이트</b> · 수동 개입 0</td>
</tr>
</tbody>
</table>

> 🏗️ [**인터랙티브 아키텍처 쇼케이스**](https://gazuua.github.io/apex_pipeline/) — 전체 생태계 아키텍처, HTTP 대시보드, 플로우차트, 상태 머신, 라이프사이클 상세
...
- **v0.6.5.0 — 옵저빌리티 + 코어 강화 + 대시보드 관찰 전용화**
  ...
  - NUMA 바인딩 + Core Affinity — 물리 코어 1:1 핀닝, P/E 코어 분류, NUMA set_mempolicy, topology auto-discovery (BACKLOG-40)
...
- **도구: notification 시스템 제거 (BACKLOG-164)**
  - 알림/ack/probe 전면 삭제, 상태 전이 + FIXING 백로그 체크만 유지

- **도구: 백로그 JSON 통합 (BACKLOG-163)**
  - DB(SQLite)가 source of truth, JSON은 git 백업 전용
  - 레거시 BACKLOG.md → BACKLOG.json export 체계 완성

- **도구: MSVC release 빌드 C4189 미사용 변수 경고 수정 (PR #125)**

- **도구: 백로그 정합성 강화 (BACKLOG-160)**
  - ExportHistory: RESOLVED 항목 → BACKLOG_HISTORY.md 자동 이관 (중복 방지, 마커 복원)
  - backlog update CLI: 필드별 메타 수정 + `--position` 섹션 내 재배치
  - validate-backlog hook: BACKLOG 파일 직접 편집 차단 (대소문자 무관)
  - MergePipeline 자동 커밋: export→commit→push→checkout main→IPC 일괄 처리
  - backlog export: stdout 출력 → 파일 직접 쓰기로 전환
...
- **도구: apex-agent Go 백엔드 완전 재작성 (#126)**
  ...
  - 6개 hook 게이트 + E2E 테스트 14 패키지 전체 PASS + CI Go 파이프라인
