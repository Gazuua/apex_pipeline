...
`apex_core/`(코어), `apex_shared/`(공유 라이브러리+어댑터), `apex_services/`(MSA), `apex_infra/`(Docker/K8s — `apex-pipeline` + `apex-observability` Helm 차트), `apex_tools/`(CLI/스크립트/auto-review), `docs/`(전체 문서 중앙 집중)
...
- **현재**: v0.6.5 — Loki+Vector+Grafana 옵저빌리티 — KafkaSink 제거, log-svc C++ 폐기, apex-observability Helm 차트(BACKLOG-199). CI/CD 고도화 + SocketBase virtual interface(BACKLOG-133) + HttpServerBase/AdminHttpServer 런타임 로그 레벨(BACKLOG-179) + BlockingTaskExecutor CPU offload(BACKLOG-146) + SecureString 메모리 제로화+constant-time 비교+SSO 버퍼 제로화(BACKLOG-135,245,246) + ESO/AWS SM 연동(BACKLOG-198) + ESO per-service RBAC(BACKLOG-252). Gateway RL RedisAdapter 라이프사이클 통합(BACKLOG-250) + rate_limiter_ atomic 제거(BACKLOG-251). full auto-review 22건 수정(SpscQueue 레이스, Redis 콜백, refresh token 트랜잭션, Helm 보안 강화). Helm Rollout CRD 지원, 3단계 검증 파이프라인. K8s Helm logging.file 비활성화(BACKLOG-255). 보안 소탕: Kafka manual commit(BACKLOG-247) + Gateway 시스템 메시지 인증(BACKLOG-249) + auth exempt rate limit(BACKLOG-248) + K8s NetworkPolicy(BACKLOG-253). Whisper O(1) core routing — Auth SessionStore core_id 저장 + Gateway 단일 코어 직접 전달(BACKLOG-149). Acceptor per-IP 연결 제한 — owner-shard 패턴, cross_core_call 기반 no-locking(BACKLOG-256). 대시보드 관찰 전용화 — 세션 서버+Branches 페이지+local_branches 제거(BACKLOG-260). NUMA 바인딩+Core Affinity — 물리 코어 1:1 핀닝, P/E 분류, NUMA set_mempolicy, topology discovery(BACKLOG-40)
- **도구**: apex-agent Go 백엔드 — HTTP 대시보드(`localhost:7600`, 4개 페이지: Dashboard/Backlog/Handoff/Queue, 관찰 전용), 빌드/머지 큐, 백로그 DB+CLI, 핸드오프 상태 머신, cleanup, 일별 로그 분할, 백로그 blocked_reason(CLI show 출력), queue build 콘솔 실시간 출력. 주요 완료: PR #126(재작성), #130(대시보드), #145(큐 히스토리), #150(IPC), #156(stale guard), #162(RESOLVED 원복 방지), #164(로직 강화), #167(로그 분할), #169(idle timeout 제거+auto-restart), #174(notify merge 통합), #176(Go build.bat 허용+watchdog), #213(세션 서버+Branches 제거, 대시보드 관찰 전용화 BACKLOG-260)
