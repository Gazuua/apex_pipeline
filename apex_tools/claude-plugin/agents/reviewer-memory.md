...
- [ ] 스택 할당 객체의 수명이 비동기 콜백 수명보다 긴가?
- [ ] 코루틴 프레임 내 객체가 co_await 이후에도 유효한가?
- [ ] 포인터/참조를 외부 컨테이너에 저장할 때 소유권이 명확한가?
- [ ] 장시간 운영 시 메모리 증가 패턴이 없는가? (shrink 정책)
...
4. **Confidence >= 50이면 보고** — 메모리 이슈는 심각도가 높으므로 보수적으로 보고
