...
4. **CI 대기**
   ...
   **CI 결과 확인 절차 (2단계 필수)**:
   ...
   2. **완료 후 반드시** `gh run view {run-id}` — 전체 run 상태 확인
      - 첫 줄의 `X` (실패) 또는 `✓` (성공) 체크
      - 개별 job별 상태 확인 (모든 job이 `✓`여야 통과)
   3. **`gh run watch` 출력의 tail만으로 성공 여부를 판단하지 말 것** — 여러 job 중 마지막에 출력된 성공 job만 보이고, 앞서 실패한 job이 누락되는 오판 발생 가능
...
1. **CI 실패 분석**
   ...
   **CI 결과 확인 절차 (2단계 필수)** — Phase 3에서 CI 실패로 진입했더라도, 수정 후 CI 재실행 시 동일하게 적용:
   1. `gh run watch {run-id}` — 완료 대기 (`run_in_background: true`)
   2. **완료 후 반드시** `gh run view {run-id}` — 전체 run 상태 확인
      - 첫 줄의 `X` (실패) 또는 `✓` (성공) 체크
      - 개별 job별 상태 확인 (모든 job이 `✓`여야 통과)
...
1. **팀 해산**
   ...
   coordinator의 shutdown-report에 zombie 목록이 있거나, TeamDelete가 실패하는 경우의 안전장치:

   1. coordinator의 `[shutdown-report]`에서 zombie 목록 확인
   2. TeamDelete 시도
   3. TeamDelete 실패 시 (active member 존재):
      1. `~/.claude/teams/{team-name}/config.json`을 Read로 읽기
      2. 남아있는 좀비 멤버를 config.json의 `members` 배열에서 직접 제거
         - **인코딩 주의**: config.json 읽기/쓰기 시 `encoding='utf-8'` 보장 (한글 에이전트명 깨짐 방지)
         - JSON 구조를 파싱하여 해당 멤버만 정확히 제거 (수동 문자열 치환 금지)
      3. TeamDelete 재시도
      4. 재시도도 실패 시: config.json의 `members` 배열을 빈 배열(`[]`)로 초기화 후 TeamDelete 최종 시도
   4. 모든 fallback 실패 시 유저에게 수동 정리 안내:
      ```
      [경고] 팀 정리 실패 — ~/.claude/teams/{team-name}/ 디렉토리를 수동 삭제하세요.
      ```
