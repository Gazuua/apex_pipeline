...
> **구조**: 메인이 coordinator + 리뷰어 11명 = 총 12명을 같은 팀에 스폰한다.
...
2. **12명 전부 스폰 (Agent x 12, 병렬)**

   메인이 Agent 도구로 coordinator 1명 + 리뷰어 11명 = 총 12명을 같은 팀에 스폰한다.
   ...
   **A) coordinator 스폰:**
   ...
   > **coordinator 시작 타이밍**: coordinator는 스폰 직후 컨텍스트 수집(에이전트 정의, config, smart_skip 등)만 수행하고,
   > **메인의 start 시그널을 수신할 때까지 리뷰어 assign을 시작하지 않는다.**
   > start 시그널은 모든 리뷰어가 팀에 등록된 후 별도 SendMessage로 전송된다.
...
3. **전원 등록 확인**
   ...
   12명 스폰 후, 전원이 팀에 등록되었는지 확인한다.
   `~/.claude/teams/auto-review/config.json`을 Read로 읽어서 `members` 배열 검증:
   - coordinator 1명 + 리뷰어 11명 = **총 12명** 전원 등록 확인
   - 미등록 멤버가 있으면 **5초 후 재확인** (최대 3회)
   - 3회 재확인 후에도 미등록 멤버가 있으면 프로세스 중단 + 유저에게 보고

4. **coordinator에게 start 시그널 전송 (SendMessage)**

   전원 등록 확인 후, coordinator(`review-coordinator`)에게 SendMessage로 start 시그널을 전송한다:
   ```
   [start]
   all_reviewers_registered: true
   reviewer_count: 11
   ```
   이 시그널을 수신한 coordinator는 리뷰어 assign을 시작한다.
...
1. **팀 해산**
   ...
   report 수신 후 팀을 정리한다:
   1. coordinator에게 SendMessage로 `shutdown_request` 전송
   2. coordinator가 모든 리뷰어에게 `shutdown_request`를 보내고, 리뷰어 전원 종료 후 자신도 승인
   3. 모든 teammate 종료 확인 후 TeamDelete로 팀 정리
