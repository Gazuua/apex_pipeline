...
> **구조**: 메인이 리뷰어 11명을 먼저 스폰 → 전원 등록 확인 → coordinator를 마지막에 스폰한다.
...
2. **리뷰어 11명 먼저 스폰 (Agent x 11, 병렬)**

   메인이 Agent 도구로 리뷰어 11명을 같은 팀에 스폰한다.
   11명 모두 독립적인 Agent 호출이므로 **병렬 스폰** 가능하다.
...
3. **전원 등록 확인**
   ...
   리뷰어 11명 스폰 후, 전원이 팀에 등록되었는지 확인한다.
...
4. **coordinator 스폰 (Agent x 1)**
   ...
   리뷰어 11명 전원 등록 완료. 컨텍스트 수집 후 즉시 assign 시작하라.
   ```

   > **coordinator 시작 타이밍**: 리뷰어 11명이 전원 등록된 후 coordinator를 스폰하므로,
