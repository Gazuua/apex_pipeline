...
`$LOCALAPPDATA/apex-agent/config.toml` (Windows) / `~/.config/apex-agent/config.toml` (Linux). 파일 없으면 전부 기본값으로 동작.

```bash
apex-agent config init   # 기본 config.toml 생성
apex-agent config show   # 현재 설정 출력
```

| 섹션 | 주요 필드 | 기본값 | 설명 |
|------|-----------|--------|------|
| `[daemon]` | `socket_path` | 플랫폼 자동 | Named Pipe / Unix Socket |
| `[store]` | `db_path` | `$LOCALAPPDATA/apex-agent/apex-agent.db` | SQLite 경로 |
| `[queue]` | `stale_timeout`, `poll_interval` | `1h`, `1s` | 빌드 큐 설정 |
...
apex-agent backlog update ID --title "..." --severity MAJOR --description "..." [--related "N,M"] [--position N] [--blocked "사유"]
...
```bash
apex-agent backlog update 146 --blocked "bcrypt: hard timeout vs sliding 중 택1"  # 설정
...
| backlog | v4 | blocked_reason 컬럼 추가 |
