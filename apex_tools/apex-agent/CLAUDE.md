...
**자동 export**: `add`, `update`, `resolve`, `release`, `fix` 명령은 DB 변경 후 `docs/BACKLOG.json`을 자동 export한다. 별도 `backlog export` 실행 불필요. export 실패 시 warning만 출력 (mutation 자체는 성공 유지).
