...
- shared_ptr 기반 세션 수명 (코루틴 안전성) → Phase 5.5에서 intrusive_ptr + SlabPool 전환 예정
...
Phase 5 (완료)
    │
Phase 5.5 — 코어 프레임워크 성능적 완성도
    │  Tier 0:   벤치마크 인프라 (Google Benchmark)
    ...
    └──→ Phase 7 (데이터 체인) ──┘         │
              │                            ▼
              └──→ v0.3.0          Phase 8b (Gateway + Auth) → v0.4.0
                                           │
                                   Phase 8.5 (파이프라인 E2E 통합 테스트)
                                           │
                                   Phase 9 (운영 인프라) → v0.5.0
                                           │
                                   Phase 10 (최종 통합 + 게임 클라이언트 + AWS) → v1.0.0
```

| Phase | 버전 | 내용 | 의존 |
|-------|------|------|------|
| **5.5** | - | **코어 프레임워크 성능적 완성도**: 벤치마크 + 에러 정비 + 핫패스/메모리/구조 최적화 | Phase 5 |
| 6 | v0.3.0 | Kafka 체인: Kafka 어댑터 → KafkaSink + trace_id | Phase 5.5 |
| 7 | v0.3.0 | 데이터 체인: Redis 어댑터 ∥ PG 어댑터 → Connection Pool | Phase 5.5 |
...
| 8.5 | - | 파이프라인 E2E 통합 테스트 | Phase 8b |
| 9 | v0.5.0 | 운영 인프라: 메트릭 + Docker + K8s + CI/CD 고도화 | Phase 8.5 |
| 10 | v1.0.0 | 최종 통합: E2E 검증 + 부하 테스트 + 게임 클라이언트(Android/Kotlin) + AWS 배포 | Phase 9 |
...
#### Phase 5.5: 코어 프레임워크 성능적 완성도
- [x] **Tier 0**: 벤치마크 인프라 (Google Benchmark vcpkg + micro/integration 벤치마크 + baseline 측정)
- [x] **Tier 0.5**: 에러 타입 통일 (DispatchError → ErrorCode 흡수) + async_send → Result\<void\>
- [x] **Tier 1**: 핫패스 — drain/tick 분리 (post+atomic coalescing + batch limit)
- [x] **Tier 1**: Cross-Core Message Passing 아키텍처 전환 (closure shipping → op 기반 handler dispatch)
- [x] **Tier 1**: MessageDispatcher 65536-array → boost::unordered_flat_map
- [x] **Tier 1**: payload zero-copy dispatch (consume 순서 변경)
- [ ] **Tier 1.5**: E2E echo 부하 테스터 + JSON before/after 비교 도구
- [ ] **Tier 2**: SessionPtr shared_ptr → intrusive_ptr (non-atomic refcount)
- [ ] **Tier 2**: Session SlabPool 전환 + sessions_/timer_to_session_ → flat_hash_map
...
#### Phase 8.5: 파이프라인 E2E 통합 테스트
- [ ] Gateway → Kafka → Service → Redis/PG 전체 경로 E2E 통합 테스트
...
- [ ] 모니터링 대시보드 구성
- [ ] 게임 클라이언트 (Android/Kotlin) + AWS 배포
