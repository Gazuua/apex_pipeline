...
| **Connection Pool** | Service → Redis/PostgreSQL | 커넥션 생성 비용 제거 | v0.4.4.0 |
...
| **Graceful Shutdown** | 전 서비스 | SIGTERM → acceptor 중지 → **어댑터 drain (Kafka flush, Redis/PG idle close)** → 코어별 세션 close(코어 스레드에 비동기 post) → 세션 drain 폴링(active_sessions==0 대기, 1ms 주기) → CoreEngine stop → CoreEngine join → drain_remaining(잔여 MPSC 메시지 소비) → 서비스 정지 → **어댑터 close (Kafka Producer/Consumer 해제, Redis/PG 풀 close_all)** → shutdown_logging() → 종료. drain 타임아웃: ADR-05에서 기본값 25초(K8s 30초 대비 5초 여유)로 설계 확정, v0.2.0.0에서 구현. shutdown 후 로깅 시도는 spdlog::get() null 체크로 방어 |
...
| **Backpressure** | MPSC 큐 | max_capacity 초과 시 enqueue 거부 (`ErrorCode::BufferFull`). 80% 슬로우다운/429 응답은 Gateway 구현 시 추가 예정 |
...
│   │   ├── common/                   ← AdapterBase CRTP, ConnectionPool CRTP, PoolStats
│   │   ├── kafka/                    ← KafkaProducer, KafkaConsumer, KafkaAdapter, KafkaSink
│   │   ├── redis/                    ← RedisConnection, RedisPool, RedisAdapter, HiredisAsioAdapter
│   │   └── pg/                       ← PgConnection, PgResult, PgPool, PgAdapter
...
- intrusive_ptr 기반 세션 수명 (non-atomic refcount, per-core 단일 스레드 보장) + SlabPool 할당
...
| `cross_core_post_msg(engine, src, dst, op, data)` | fire-and-forget | message passing 기반 제로할당 전달 |
| `broadcast_cross_core(engine, src, op, payload)` | broadcast | SharedPayload로 전 코어 전달 |

구현: CoreMessage(CrossCoreOp + source_core + uintptr_t) → MPSC 큐 → CrossCoreDispatcher handler dispatch. SharedPayload(atomic refcount)로 immutable 데이터 공유. 기존 closure 방식은 LegacyCrossCoreFn op으로 호환 유지.
...
> 총 41개 단위 테스트 PASS. 통합 테스트는 docker-compose 환경에서 실행 (APEX_BUILD_INTEGRATION_TESTS=ON).
...
| v0.4.2.0 | 중 | Redis 어댑터 (redis-plus-plus async) | v0.3 |
| v0.4.3.0 | 중 | PostgreSQL 어댑터 (libpq → Asio) | v0.3 |
| v0.4.4.0 | 중 | Connection Pool (코어별 독립, health check) | v0.4.2+3 |
| **v0.5.0.0** | **대** | **서비스 체인** | |
| v0.5.1.0 | 중 | WebSocket 프로토콜 (Boost.Beast, ProtocolBase CRTP, ping/pong) | v0.3 |
| v0.5.2.0 | 중 | Gateway (TLS, JWT, 블룸필터, Kafka 라우팅, Rate Limiting) | v0.4 + v0.5.1 |
| v0.5.3.0 | 중 | Auth 서비스 (JWT 발급/검증/블랙리스트) | v0.4 + v0.5.1 |
| v0.5.4.0 | 중 | 파이프라인 E2E 통합 테스트 | v0.5.2+3 |
| **v0.6.0.0** | **대** | **운영 인프라** | |
...
#### 백로그 (해당 버전에서 흡수)
- [ ] Circuit Breaker (→ v0.5.2 Gateway)
- [ ] Dead Letter Queue (→ v0.4.1 Kafka)
