...
| Benchmark | Google Benchmark | gbenchmark | Phase 5.5 |
...
- drain_timer 기반 주기적 inbox 소비 + 콜백 실행 → Phase 5.5에서 post()+atomic drain + tick 분리 전환 예정
...
> **Phase 5.5 전환 예정**: closure shipping → message passing 아키텍처. CrossCoreOp 기반 handler dispatch + immutable shared payload. cross_core_call의 CAS race를 single-thread 직렬화로 근본 제거.
...
#### Phase 5.5: 코어 프레임워크 성능적 완성도
- [ ] **Tier 0**: 벤치마크 인프라 (Google Benchmark vcpkg + micro/integration 벤치마크 + baseline 측정)
- [ ] **Tier 0.5**: 에러 타입 통일 (DispatchError → ErrorCode 흡수) + async_send → Result\<void\>
- [ ] **Tier 1**: 핫패스 — drain/tick 분리 (post+atomic coalescing + batch limit)
- [ ] **Tier 1**: Cross-Core Message Passing 아키텍처 전환 (closure shipping → op 기반 handler dispatch)
- [ ] **Tier 1**: MessageDispatcher 65536-array → boost::unordered_flat_map
- [ ] **Tier 1**: payload zero-copy dispatch (consume 순서 변경)
