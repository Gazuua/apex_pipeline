...
│   ├── auth-svc/                     ← 인증 서비스
│   ├── chat-svc/                     ← 채팅 서비스
│   ├── log-svc/                      ← (제거됨 — Loki+Vector로 대체, BACKLOG-199)
...
│   └── k8s/                          ← Helm charts (apex-pipeline: 서비스 3개 + 인프라 3종, apex-observability: Loki+Vector+Grafana)
├── apex_tools/                       ← CLI, 도구, git-hooks, auto-review
│   ├── apex-agent/                   ← Go 바이너리 (빌드큐, 핸드오프, 백로그, hook, cleanup, 플러그인 셋업)
...
│   ├── apex-auto-review/                ← Claude 플러그인 (auto-review 오케스트레이터 + 7개 리뷰어 에이전트)
...
| Monitoring | Prometheus + Grafana | 자체 구현 (Beast HTTP + MetricsRegistry) | v0.6.1.0 |
...
public:
    ...
    awaitable<Result<void>> on_echo(SessionPtr session, uint32_t msg_id,
        ...
        // ... 응답 빌드 ...
        auto header = WireHeader::make(msg_id + 1, builder.GetSize());
...
> **도구: ~~워크스페이스 모듈~~ + blocked_reason Phase 1 (PR #195, BACKLOG-238,239)** — ~~workspace 모듈(디렉토리 스캔, REST API, local_branches 테이블)~~, backlog blocked_reason 필드 + ⚠ 뱃지. *(워크스페이스 모듈·session config·local_branches는 BACKLOG-260, PR #213에서 제거됨)*

> **도구: ~~세션 서버 Phase 2-3~~ (BACKLOG-238,239)** — ~~ConPTY 기반 독립 프로세스 세션 서버(:7601), WebSocket 실시간 터미널, Watchdog 자동 재시작, session CLI(run/start/stop/status/send), 대시보드 /branches 페이지(xterm.js 웹 터미널), 리버스 프록시(:7600→:7601), Workspace REST API~~. *(세션 서버·Branches 페이지·워크스페이스 모듈 전체가 BACKLOG-260, PR #213에서 제거됨. 대시보드 4페이지 체제: Dashboard/Backlog/Handoff/Queue)*

> **도구: ~~/branches 검증 + 외부 세션 감지~~ (BACKLOG-242, PR #206)** — ~~외부 Claude Code 세션 감지(Hook+mtime 폴백+좀비 정리), ref count 중복 세션 추적, EXTERNAL/MANAGED Sync 차단, plugin reload 방지(multi-workspace 경로 불일치). /branches CSS 스타일링+HTMX 깜빡임 방지, 워크스페이스 정렬 정규화~~. backlog show blocked_reason CLI 출력, queue build 콘솔 실시간 출력. Go 1.22 ServeMux 라우트 충돌 panic 수정, build.bat 괄호 파싱 버그 수정. *(세션·워크스페이스·/branches 관련 기능은 BACKLOG-260, PR #213에서 제거됨. blocked_reason CLI·queue build 콘솔·ServeMux/build.bat 수정은 유지)*
...
v0.5.0.0 (완료) ── Wave 1: Protocol concept + 어댑터 회복력
    └──→ v0.5.4.0 (완료) ── Wave 2: 서비스 체인
         ...
         [도구] 워크스페이스 모듈 + blocked_reason Phase 1 (PR #195, BACKLOG-238,239 — 디렉토리 스캔, REST API, blocked ⚠ 뱃지) ← 워크스페이스·local_branches 제거됨(BACKLOG-260)
         [도구] 세션 서버 Phase 2-3 (BACKLOG-238,239 — ConPTY 독립 프로세스 :7601, WebSocket, Watchdog, session CLI, 대시보드 /branches xterm.js, 리버스 프록시, Workspace REST API) ← 전체 제거됨(BACKLOG-260)
         ...
         [도구] /branches 검증 + 외부 세션 감지 (BACKLOG-242, PR #206 — Hook+mtime 폴백, ref count, Sync 차단, CSS/HTMX 개선, build.bat/ServeMux 수정) ← 세션·워크스페이스·/branches 제거됨(BACKLOG-260)
...
- [x] ~~KafkaSink (spdlog sink 추가)~~ → v0.6.5에서 제거 (Loki+Vector stdout 수집으로 대체, BACKLOG-199)
...
- [x] Prometheus 메트릭 노출
- [x] Docker 멀티스테이지 빌드
- [x] K8s manifests + Helm (umbrella+library chart, 2-release, expand_env, Bitnami 하이브리드, minikube 검증)
- [x] CI/CD 고도화 (이미지 빌드 + 배포, docker-compose 통합 테스트)
- [x] 옵저빌리티 스택: Loki+Vector+Grafana (KafkaSink 제거, log-svc C++ 폐기, apex-observability Helm 차트, BACKLOG-199)
