...
│   ├── claude-plugin/                ← Claude 플러그인 (오케스트레이터 + 12개 리뷰어 에이전트)
...
| Auth | JWT + Bloom Filter | jwt-cpp | v0.5.2.0 |
...
| v0.5.2.1 | 소 | Gateway 회복력 (Rate Limiting Token Bucket + Circuit Breaker) | v0.5.2 |
| v0.5.3.0 | 중 | Auth 서비스 (JWT 발급/검증/블랙리스트, Redis 세션) | v0.4 + v0.5.2 |
| v0.5.3.1 | 소 | Logic Service 스캐폴딩 (최소 1개 비즈니스 서비스) | v0.4 + v0.5.1 |
...
로드맵 마일스톤의 실행 단위. 같은 코드 영역 + 같은 아키텍처 결정권 내의 작업을 하나로 묶어 1 워크트리 / 1 PR / auto-review 1회로 처리한다.
...
| **2** | **D. 서비스 체인** | Gateway MVP (TLS+JWT+Kafka라우팅), Auth 서비스, Rate Limiting, Logic Service 스캐폴딩 | `apex_services/` | Wave 1 |
| **3** | **E-2. E2E 통합 테스트** | Client → Gateway → Auth → Logic → Redis/PG 시나리오, Error Path 테스트 | `tests/e2e/` | Wave 2 |
| **3** | **F. 운영 인프라** | Prometheus 메트릭, Docker 멀티스테이지, K8s Helm, CI/CD 고도화 | `apex_infra/`, `.github/` | Wave 2 |

**규칙:**
- Wave 내 묶음은 **병렬 실행** (에이전트 팀별 파일 경계 준수)
- Wave 간은 **순차 실행** (앞 Wave 완료 + auto-review 통과 후 다음 Wave)
- 빌드는 메인 에이전트/coordinator만 수행 (워커는 코드 수정만)
...
- **어떻게?** SIGTERM → acceptor 중지 → 코어별 세션 close(코어 스레드에 비동기 post) → 세션 drain 폴링(active_sessions==0 대기, 1ms 주기) → 어댑터 drain(새 요청 거부, is_ready=false) → 서비스 on_stop() → CoreEngine stop → CoreEngine join → drain_remaining(잔여 MPSC 메시지 소비) → 어댑터 close(Kafka flush, Redis/PG 풀 close_all) → shutdown_logging → 종료
