...
```bash
git show HEAD:apex_tools/claude-plugin/agents/reviewer-memory.md | head -30
git show HEAD:apex_tools/claude-plugin/agents/reviewer-concurrency.md | head -30
# ... 나머지도 동일
```

(Task 3에서 삭제 전이라면 직접 Read, 삭제 후라면 git show 사용)
...
```bash
grep -r "coordinator" apex_tools/ --include="*.md" -l
grep -r "SendMessage" apex_tools/ --include="*.md" -l
grep -r "TeamCreate\|TeamDelete" apex_tools/ --include="*.md" -l
grep -r "reviewer-memory\|reviewer-concurrency\|reviewer-api\|reviewer-architecture" apex_tools/ --include="*.md" -l
grep -r "reviewer-test-coverage\|reviewer-test-quality\|reviewer-security\b\|reviewer-infra\b" apex_tools/ --include="*.md" -l
grep -r "reviewer-cross-cutting" apex_tools/ --include="*.md" -l
