...
CLI 호출 → Dial() 시도
  ├─ 성공 → 요청 전송 → 응답 수신
  └─ 실패 → daemon start (백그라운드) → 재시도 (3회, 100ms 간격)

데몬 내부:
  시작: PID 파일 작성 → SQLite 열기 (WAL) → 모듈 등록/시작 → IPC 리스너
  실행: IPC 수신 → Router → Module → 응답
  유휴: 마지막 요청 후 30분 → 자동 종료
...
| **5** | 나머지 (cleanup, context, enforce-rebase) | 모든 bash 제거, Go 단일 운영 | 나머지 전체 |
