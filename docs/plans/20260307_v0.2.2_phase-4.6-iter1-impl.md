...
Run: `cmd.exe //c "D:\\.workspace\\BoostAsioCore\\build.bat" debug 2>&1`
...
**Files:**
- Create: `core/include/apex/core/protocol_base.hpp`
- Create: `core/include/apex/core/tcp_binary_protocol.hpp`
- Modify: `core/src/server.cpp:134-152` (process_frames에서 TcpBinaryProtocol 사용)
- Modify: `core/src/server.cpp:1` (include 추가)
- Test: `core/tests/unit/test_tcp_binary_protocol.cpp` (신규)
- Modify: `core/tests/CMakeLists.txt` (테스트 파일 추가)
...
Create: `core/tests/unit/test_tcp_binary_protocol.cpp`
...
`core/tests/CMakeLists.txt`에 `unit/test_tcp_binary_protocol.cpp`를 소스 목록에 추가.
...
```bash
git add core/include/apex/core/protocol_base.hpp \
        core/include/apex/core/tcp_binary_protocol.hpp \
        core/tests/unit/test_tcp_binary_protocol.cpp \
...
**Files:**
- Modify: `core/include/apex/core/message_dispatcher.hpp` (Handler 타입 + dispatch 반환 타입)
- Modify: `core/src/message_dispatcher.cpp` (dispatch 구현)
- Modify: `core/include/apex/core/service_base.hpp` (handle/route 시그니처)
- Modify: `core/src/server.cpp` (process_frames 에러 전파)
- Modify: `core/tests/unit/test_message_dispatcher.cpp` (8개 테스트 시그니처)
- Modify: `core/tests/unit/test_service_base.cpp` (핸들러 시그니처)
- Modify: `core/tests/unit/test_flatbuffers_dispatch.cpp` (핸들러 시그니처 + 검증 실패 기대값 변경)
- Modify: `core/examples/echo_server.cpp` (핸들러 반환 타입)
- Modify: `core/examples/chat_server.cpp` (핸들러 반환 타입)
- Create: `core/tests/integration/test_error_pipeline.cpp` (에러 자동 전파 E2E)
- Modify: `core/tests/CMakeLists.txt` (통합 테스트 추가)

이 태스크는 서브태스크로 분할한다.

### Step 3-1: MessageDispatcher Handler 타입 변경

`core/include/apex/core/message_dispatcher.hpp` 수정:
...
`core/src/message_dispatcher.cpp` 전체:
...
`core/include/apex/core/service_base.hpp` 수정 (handle과 route 메서드):
...
`core/src/server.cpp` process_frames 변경 (134행~):
...
`core/tests/unit/test_message_dispatcher.cpp` 전체:
...
`core/tests/unit/test_service_base.cpp` 전체:
...
`core/tests/unit/test_flatbuffers_dispatch.cpp` 전체:
...
`core/examples/echo_server.cpp` EchoService 핸들러 시그니처 변경:
...
`core/examples/chat_server.cpp` ChatService 핸들러 시그니처 변경:
...
```bash
git add core/include/apex/core/message_dispatcher.hpp \
