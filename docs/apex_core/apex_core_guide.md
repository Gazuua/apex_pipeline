...
void on_wire(WireContext& ctx) override {
    ...
    auto* other = ctx.local_registry.get<OtherService>();
...
Signal (SIGINT/SIGTERM)
 → Server::stop()
 → begin_shutdown()
    → Adapter 정지 (Kafka consumer 스레드 종료)
    → on_stop() 호출 (서비스 정리)
    → [D7] outstanding 코루틴 카운터 0 대기 (drain_timeout 내)
    → CoreEngine 정지 (코어 스레드 join)
    → Server 소멸 (cross-core 자원, 어댑터, 서비스 역순 파괴)
```
...
shared-nothing 원칙에 예외 없음. 모든 뮤텍스는 금지.
