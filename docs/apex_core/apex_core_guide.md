...
Signal (SIGINT/SIGTERM)
 → Server::stop()
 → finalize_shutdown()
    0. metrics_http_server_.stop() + admin_http_server_.stop()
       (HTTP 서버 먼저 정지 — 외부 요청 수신 차단)
    ...
    6.5. blocking_executor_->shutdown()
         (BlockingTaskExecutor 스레드 풀 join — 진행 중 CPU-bound 작업 완료 대기)
    7. [D3] Globals clear
...
### AdminHttpServer — 런타임 로그 레벨 동적 전환

`AdminConfig`를 통해 관리용 HTTP 서버를 활성화할 수 있다:
...
- Shutdown Step 0에서 `admin_http_server_.stop()` 호출로 자동 정지 (metrics HTTP 서버와 함께)
