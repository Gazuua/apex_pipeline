...
기존 설계 원칙 문서(Why Document)의 연장선이며, 프레임워크 세부 설계에 초점을 맞춘다.
...
### 맥락
- Gateway 패턴: 외부 TLS 종단, 내부 평문 (설계 문서에서 확정)
- 인증 토큰 검증 방식 미결정
...
4. "원칙을 알되 현실적 예외를 인지한다"는 설계 태도가 프로젝트 전반에 좋은 메시지
...
### 맥락
- 초기 설계 원본에서 "io_uring 백엔드 자동 활성화" 명시
...
6. "io_uring의 한계를 알면서 선택하지 않은" 판단이 설계 깊이를 보여줌
...
4. 설계 문서에 정직하게 범위를 명시 — 프로젝트 신뢰도 상승
