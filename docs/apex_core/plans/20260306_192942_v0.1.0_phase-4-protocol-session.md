...
| 파일 | 이유 |
|------|------|
| `include/apex/core/service_base.hpp` | CRTP 패턴, handle() 등록, 디스패처 |
| `include/apex/core/core_engine.hpp` | CoreContext, CoreEngine, MPSC 메시지 라우팅 |
| `include/apex/core/wire_header.hpp` | 10바이트 헤더 형식, wire_flags |
| `include/apex/core/frame_codec.hpp` | RingBuffer 기반 프레임 디코드/인코드 |
| `include/apex/core/timing_wheel.hpp` | EntryId, schedule/cancel/tick API |
| `include/apex/core/slab_pool.hpp` | TypedSlabPool<T> 세션 할당용 |
| `include/apex/core/ring_buffer.hpp` | writable/commit_write/linearize API |
| `examples/echo_server.cpp` | 현재 세션 패턴 (EchoSession) |
| `tests/unit/CMakeLists.txt` | apex_add_unit_test() 함수 패턴 |
...
**파일:**
- 생성: `include/apex/core/result.hpp`
- 생성: `include/apex/core/error_code.hpp`
- 생성: `include/apex/core/session.hpp`
- 생성: `include/apex/core/session_manager.hpp`
- 생성: `schemas/echo.fbs`
- 생성: `schemas/error_response.fbs`
- 생성: `schemas/heartbeat.fbs`
- 생성: `src/session.cpp` (빈 스텁)
- 생성: `src/session_manager.cpp` (빈 스텁)
- 수정: `CMakeLists.txt` (새 소스 추가, FlatBuffers 코드젠)
- 수정: `include/apex/core/service_base.hpp` (route<T> 선언 추가)
- 생성: `tests/unit/test_session.cpp` (주석 처리)
- 생성: `tests/unit/test_session_manager.cpp` (주석 처리)
- 생성: `tests/unit/test_error_propagation.cpp` (주석 처리)
- 생성: `tests/unit/test_flatbuffers_dispatch.cpp` (주석 처리)
...
```cpp
// tests/unit/test_error_propagation.cpp
#include <gtest/gtest.h>
...
**파일:**
- 수정: `include/apex/core/service_base.hpp` (route<T> 추가)
- 수정: `tests/unit/test_flatbuffers_dispatch.cpp` (테스트 구현)
- 참고: `schemas/*.fbs` (생성된 헤더 확인)
...
`include/apex/core/service_base.hpp` 상단에 인클루드 추가:
...
```bash
git add include/apex/core/service_base.hpp \
        tests/unit/test_flatbuffers_dispatch.cpp
...
**파일:**
- 수정: `src/session.cpp` (Session 구현)
- 수정: `src/session_manager.cpp` (SessionManager 구현)
- 수정: `tests/unit/test_session.cpp` (테스트 구현)
- 수정: `tests/unit/test_session_manager.cpp` (테스트 구현)
...
```bash
git add src/session.cpp src/session_manager.cpp \
        include/apex/core/session_manager.hpp \
        tests/unit/test_session.cpp \
        tests/unit/test_session_manager.cpp
...
**파일:**
- 수정: `tests/unit/test_error_propagation.cpp` (테스트 구현)
- 생성: `include/apex/core/error_sender.hpp` (에러 프레임 빌더)
- 생성: `src/error_sender.cpp`
- 수정: `CMakeLists.txt` (error_sender.cpp 추가)
...
```bash
git add include/apex/core/error_sender.hpp \
        include/apex/core/error_code.hpp \
        include/apex/core/result.hpp \
        src/error_sender.cpp \
        CMakeLists.txt \
