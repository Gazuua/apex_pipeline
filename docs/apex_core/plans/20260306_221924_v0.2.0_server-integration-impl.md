...
| 파일 | 이유 |
|------|------|
| `include/apex/core/message_dispatcher.hpp` | Handler 타입, dispatch() 시그니처 변경 |
| `include/apex/core/service_base.hpp` | CRTP, handle(), route<T>() 시그니처 변경 |
| `src/message_dispatcher.cpp` | dispatch() 구현 변경 |
| `include/apex/core/session.hpp` | SessionPtr 타입 |
| `include/apex/core/session_manager.hpp` | for_each() 추가 |
| `examples/echo_server.cpp` | Server 기반으로 재작성 대상 |
| `tests/unit/test_message_dispatcher.cpp` | 시그니처 수정 대상 |
| `tests/unit/test_service_base.cpp` | 시그니처 수정 대상 |
| `tests/unit/test_flatbuffers_dispatch.cpp` | 시그니처 수정 대상 |
| `tests/integration/test_pipeline_integration.cpp` | 시그니처 수정 대상 |
...
**파일:**
- 수정: `include/apex/core/message_dispatcher.hpp`
- 수정: `src/message_dispatcher.cpp`
- 수정: `include/apex/core/service_base.hpp`
- 수정: `include/apex/core/session_manager.hpp`
- 수정: `src/session_manager.cpp`
- 수정: `tests/unit/test_message_dispatcher.cpp`
- 수정: `tests/unit/test_service_base.cpp`
- 수정: `tests/unit/test_flatbuffers_dispatch.cpp`
- 수정: `tests/integration/test_pipeline_integration.cpp`
- 생성: `include/apex/core/tcp_acceptor.hpp`
- 생성: `src/tcp_acceptor.cpp` (빈 스텁)
- 생성: `include/apex/core/server.hpp`
- 생성: `src/server.cpp` (빈 스텁)
- 생성: `schemas/chat_message.fbs`
- 생성: `tests/unit/test_tcp_acceptor.cpp` (빈 스텁)
- 생성: `tests/integration/test_server_e2e.cpp` (빈 스텁)
- 수정: `CMakeLists.txt`
- 수정: `tests/unit/CMakeLists.txt`
- 수정: `tests/integration/CMakeLists.txt`
...
`CMakeLists.txt`의 `APEX_SCHEMAS` 목록에 추가:
```cmake
...
`CMakeLists.txt`의 `add_library(apex_core STATIC ...)` 목록에 추가:
```cmake
    src/tcp_acceptor.cpp
    src/server.cpp
```
...
`tests/integration/CMakeLists.txt`에 추가:
```cmake
...
**파일:**
- 수정: `src/tcp_acceptor.cpp`
- 수정: `tests/unit/test_tcp_acceptor.cpp`

**다른 파일은 절대 수정하지 마세요!**

**사전 읽기:**
- `include/apex/core/tcp_acceptor.hpp` (공개 API)
...
**사전 읽기:**
- `include/apex/core/server.hpp` (공개 API)
- `include/apex/core/tcp_acceptor.hpp` (accept 콜백)
- `include/apex/core/session.hpp` (Session API)
- `include/apex/core/session_manager.hpp` (세션 관리)
- `include/apex/core/frame_codec.hpp` (try_decode, consume_frame)
- `include/apex/core/wire_header.hpp` (WireHeader 파싱)
- `include/apex/core/error_sender.hpp` (에러 프레임 빌드)
...
**파일:**
- 수정: `examples/echo_server.cpp`
- 생성: `examples/chat_server.cpp`
- 수정: `tests/integration/test_server_e2e.cpp`
- 수정: `examples/CMakeLists.txt` (chat_server 추가)
...
먼저 `examples/CMakeLists.txt`를 읽어서 구조 확인 후 chat_server 추가.
