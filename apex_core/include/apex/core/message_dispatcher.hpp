...
class MessageDispatcher
{
  public:
    ...
    using Handler = std::function<boost::asio::awaitable<Result<void>>(SessionPtr, uint32_t, std::span<const uint8_t>)>;
    ...
    [[nodiscard]] boost::asio::awaitable<Result<void>> dispatch(SessionPtr session, uint32_t msg_id,
                                                                std::span<const uint8_t> payload) const;

    [[nodiscard]] bool has_handler(uint32_t msg_id) const noexcept;
    [[nodiscard]] bool has_default_handler() const noexcept;
    [[nodiscard]] const Handler& default_handler() const noexcept
    {
