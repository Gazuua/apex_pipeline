...
public:
    ...
    Listener(uint16_t port, typename P::Config protocol_config,
             CoreEngine& engine,
        ...
        , reuseport_(reuseport)
    {
        ...
        for (auto* mgr : session_mgrs) {
            per_core_handlers_.push_back(
                std::make_unique<PerCoreHandler>(*mgr, handler_config));
        }
    }
                ...
                local.push_back(std::make_unique<TcpAcceptor>(
                    ...
                    [this, i](boost::asio::ip::tcp::socket socket) {
                        per_core_handlers_[i]->handler.accept_connection(
                            std::move(socket), engine_.io_context(i));
                    },
