...
// Measures synchronous TCP send/recv round-trip (raw socket throughput baseline)
static void BM_Session_EchoRoundTrip(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        // Server reads (동기 raw TCP — SocketBase 경유 아닌 직접 소켓 측정)
        boost::asio::read(server, boost::asio::buffer(recv_buf.data(), frame.size()));
