...
PgAdapter::query(std::string_view sql) {
...
boost::asio::awaitable<apex::core::Result<PgResult>>
PgAdapter::query(std::string_view sql, std::span<const std::string> params) {
    auto conn_result = co_await current_pool().acquire_connected();
    if (!conn_result.has_value()) {
        co_return std::unexpected(conn_result.error());
    }
...
PgAdapter::execute(std::string_view sql) {
    auto conn_result = co_await current_pool().acquire_connected();
    if (!conn_result.has_value()) {
        co_return std::unexpected(conn_result.error());
    }
...
boost::asio::awaitable<apex::core::Result<int>>
PgAdapter::execute(std::string_view sql, std::span<const std::string> params) {
    auto conn_result = co_await current_pool().acquire_connected();
    if (!conn_result.has_value()) {
        co_return std::unexpected(conn_result.error());
    }
