...
ConsumerPayloadPool::ConsumerPayloadPool(
    ...
    , max_count_(max_count)
{
    ...
    spdlog::debug("ConsumerPayloadPool: pre-allocated {} buffers ({}B each, max={})",
                  initial_count, buffer_size, max_count);
}

ConsumerPayloadPool::PayloadPtr
ConsumerPayloadPool::acquire(std::span<const uint8_t> payload) {
    ...
    return PayloadPtr(raw, [this](PayloadBuffer* p) {
        release(p);
    });
}
...
std::unique_ptr<ConsumerPayloadPool::PayloadBuffer>
ConsumerPayloadPool::create_buffer() const {
