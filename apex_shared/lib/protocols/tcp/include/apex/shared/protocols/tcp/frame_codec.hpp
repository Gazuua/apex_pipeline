...
{
  public:
    [[nodiscard]] static std::expected<Frame, FrameError> try_decode(apex::core::RingBuffer& buf);

    static void consume_frame(apex::core::RingBuffer& buf, const Frame& frame);

    [[nodiscard]] static bool encode(apex::core::RingBuffer& buf, const WireHeader& header,
