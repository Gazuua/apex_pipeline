...
static constexpr size_t TIMESTAMP_OFFSET =
    RoutingHeader::SIZE + MetadataPrefix::OFF_TIMESTAMP;  // 8 + 32 = 40

/// timestamp 8바이트를 0으로 마스킹한 뒤 비교
static bool bytes_eq_ignore_timestamp(
    ...
    const std::vector<uint8_t>& b)
{
    ...
    if (ca.size() >= TIMESTAMP_OFFSET + 8) {
        std::fill(ca.begin() + TIMESTAMP_OFFSET,
                  ca.begin() + TIMESTAMP_OFFSET + 8, 0u);
        std::fill(cb.begin() + TIMESTAMP_OFFSET,
                  cb.begin() + TIMESTAMP_OFFSET + 8, 0u);
    }
    return ca == cb;
}
...
TEST(EnvelopeBuilder, BuildIntoMultipleAllocationsPossible) {
    ...
    auto s1 = EnvelopeBuilder{}.routing(1u).metadata(0u,0ull,0u,0ull,0ull)
                               .payload(p1).build_into(alloc);
    auto s2 = EnvelopeBuilder{}.routing(2u).metadata(0u,0ull,0u,0ull,0ull)
                               .payload(p2).build_into(alloc);
...
TEST(EnvelopeBuilder, OtherFlagsPreservedInBuild) {
    const uint16_t test_flags = routing_flags::PRIORITY_HIGH
                              | routing_flags::ENCRYPTED
                              | routing_flags::DELIVERY_CHANNEL;

    auto actual = EnvelopeBuilder{}
        .routing(99u, test_flags)
        .metadata(0u, 0ull, 0u, 0ull, 0ull)
        .build();
    ...
    EXPECT_EQ(parsed->flags & ~routing_flags::HAS_REPLY_TOPIC,
              test_flags & ~routing_flags::HAS_REPLY_TOPIC);
}
