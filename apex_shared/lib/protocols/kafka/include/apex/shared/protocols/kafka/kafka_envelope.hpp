...
struct RoutingHeader
{
    ...
    [[nodiscard]] static std::expected<RoutingHeader, EnvelopeError> parse(std::span<const uint8_t> data);
...
struct MetadataPrefix
{
    ...
    [[nodiscard]] static std::expected<MetadataPrefix, EnvelopeError> parse(std::span<const uint8_t> data);
...
static constexpr size_t ENVELOPE_HEADER_SIZE = RoutingHeader::SIZE + MetadataPrefix::SIZE; // 48
...
[[nodiscard]] std::vector<uint8_t> build_full_envelope(const RoutingHeader& routing, const MetadataPrefix& metadata,
...
[[nodiscard]] size_t envelope_payload_offset(uint16_t routing_flags, std::span<const uint8_t> data);

/// Envelope에서 reply_topic을 추출. 없으면 빈 문자열.
[[nodiscard]] std::string extract_reply_topic(uint16_t routing_flags, std::span<const uint8_t> data);
