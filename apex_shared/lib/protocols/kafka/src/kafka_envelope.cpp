...
static_assert(std::endian::native == std::endian::little, "KafkaEnvelope assumes little-endian host");
...
std::expected<RoutingHeader, EnvelopeError> RoutingHeader::parse(std::span<const uint8_t> data)
{
    if (data.size() < SIZE)
    {
...
std::expected<MetadataPrefix, EnvelopeError> MetadataPrefix::parse(std::span<const uint8_t> data)
{
    if (data.size() < SIZE)
    {
        return std::unexpected(EnvelopeError::InsufficientData);
...
std::expected<std::pair<std::string, size_t>, EnvelopeError> ReplyTopicHeader::parse(std::span<const uint8_t> data)
{
...
std::vector<uint8_t> build_full_envelope(const RoutingHeader& routing, const MetadataPrefix& metadata,
                                         std::string_view reply_topic, std::span<const uint8_t> payload)
{
...
size_t envelope_payload_offset(uint16_t flags, std::span<const uint8_t> data)
{
...
std::string extract_reply_topic(uint16_t flags, std::span<const uint8_t> data)
{
    if (!(flags & routing_flags::HAS_REPLY_TOPIC))
    {
        return {};
