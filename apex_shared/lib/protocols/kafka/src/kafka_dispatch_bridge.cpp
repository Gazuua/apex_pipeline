...
KafkaDispatchBridge::dispatch(std::span<const uint8_t> raw_message) {
    ...
    auto meta_result = MetadataPrefix::parse(
        raw_message.subspan(RoutingHeader::SIZE));
    ...
    const size_t payload_offset = envelope_payload_offset(
        routing.flags, raw_message);
