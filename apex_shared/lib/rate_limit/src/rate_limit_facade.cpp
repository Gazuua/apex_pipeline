...
RateLimitFacade::RateLimitFacade(PerIpRateLimiter& ip_limiter, RedisRateLimiter& redis_limiter,
...
bool RateLimitFacade::check_ip(std::string_view ip, SlidingWindowCounter::TimePoint now)
{
    return ip_limiter_.allow(ip, now);
}

boost::asio::awaitable<apex::core::Result<RateLimitResult>> RateLimitFacade::check_user(uint64_t user_id,
                                                                                        uint64_t now_ms)
{
...
RateLimitFacade::check_endpoint(uint64_t user_id, uint32_t msg_id, uint64_t now_ms)
{
