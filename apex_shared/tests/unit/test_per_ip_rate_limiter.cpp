...
struct MockTimer {
    ...
    std::vector<std::pair<uint64_t, std::pair<std::chrono::milliseconds,
    ...
    ScheduleCallback make_schedule() {
        return [this](std::chrono::milliseconds delay,
                      std::function<void()> task) -> uint64_t {
            auto h = next_handle++;
    ...
    CancelCallback make_cancel() {
        return [this](uint64_t handle) {
            cancelled.push_back(handle);
        };
    }

    RescheduleCallback make_reschedule() {
        return [this](uint64_t handle, std::chrono::milliseconds delay) {
            rescheduled.emplace_back(handle, delay);
        };
    }
...
TEST_F(PerIpRateLimiterTest, BasicAllowDeny) {
    PerIpRateLimiter limiter(
        {.total_limit = 10, .window_size = 1s, .num_cores = 1},
...
TEST_F(PerIpRateLimiterTest, DifferentIpsIndependent) {
    PerIpRateLimiter limiter(
        {.total_limit = 5, .window_size = 1s, .num_cores = 1},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
...
TEST_F(PerIpRateLimiterTest, PerCoreLimitDivision) {
    ...
    PerIpRateLimiter limiter(
        {.total_limit = 100, .window_size = 1s, .num_cores = 4},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
...
TEST_F(PerIpRateLimiterTest, EntryCount) {
    PerIpRateLimiter limiter(
        {.total_limit = 100, .window_size = 1s, .num_cores = 1},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
...
TEST_F(PerIpRateLimiterTest, MinimumPerCoreLimit) {
    ...
    PerIpRateLimiter limiter(
        {.total_limit = 1, .window_size = 1s, .num_cores = 4},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
...
TEST_F(PerIpRateLimiterTest, TtlExpirationRemovesEntry) {
    PerIpRateLimiter limiter(
        {.total_limit = 10, .window_size = 1s, .num_cores = 1, .ttl_multiplier = 2},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
...
TEST_F(PerIpRateLimiterTest, TtlExpirationResetsCounter) {
    PerIpRateLimiter limiter(
        {.total_limit = 5, .window_size = 1s, .num_cores = 1, .ttl_multiplier = 2},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
...
TEST_F(PerIpRateLimiterTest, RescheduleOnAccess) {
    PerIpRateLimiter limiter(
        {.total_limit = 100, .window_size = 1s, .num_cores = 1, .ttl_multiplier = 2},
        mock_.make_schedule(), mock_.make_cancel(), mock_.make_reschedule());
