...
TEST(EnvelopeCodecTest, BuildAndParseRoutingHeaderRoundTrip) {
    ...
    auto parsed = envelope::RoutingHeader::parse(
...
TEST(EnvelopeCodecTest, BuildAndParseMetadataRoundTrip) {
    ...
    auto parsed = envelope::MetadataPrefix::parse(
        std::span<const uint8_t>(bytes));
    ASSERT_TRUE(parsed.has_value());
...
TEST(EnvelopeCodecTest, FullEnvelopeRoundTrip) {
    ...
    full_envelope.insert(full_envelope.end(),
    ...
    EXPECT_EQ(full_envelope.size(),
              envelope::ENVELOPE_HEADER_SIZE + payload_data.size());

    // Parse back
    auto parsed_routing = envelope::RoutingHeader::parse(
        std::span<const uint8_t>(full_envelope));
    ASSERT_TRUE(parsed_routing.has_value());
    EXPECT_EQ(parsed_routing->msg_id, 1001u);

    auto parsed_metadata = envelope::MetadataPrefix::parse(
        std::span<const uint8_t>(full_envelope).subspan(envelope::RoutingHeader::SIZE));
    ASSERT_TRUE(parsed_metadata.has_value());
    EXPECT_EQ(parsed_metadata->corr_id, 12345678u);

    // Payload extraction
    auto fbs_payload = std::span<const uint8_t>(full_envelope)
...
TEST(EnvelopeCodecTest, TooSmallRoutingHeaderFails) {
    ...
    auto result = envelope::RoutingHeader::parse(
        std::span<const uint8_t>(small));
    EXPECT_FALSE(result.has_value());
}

TEST(EnvelopeCodecTest, TooSmallMetadataFails) {
    std::vector<uint8_t> small(10);
    auto result = envelope::MetadataPrefix::parse(
        std::span<const uint8_t>(small));
    EXPECT_FALSE(result.has_value());
}
