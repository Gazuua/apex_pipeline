...
apex::core::Result<GatewayConfig> parse_gateway_config(std::string_view path)
{
    try
    {
        ...
        if (auto rpub = tbl["redis"]["pubsub"]; rpub)
        {
            ...
            cfg.redis_pubsub_password = expand_env(rpub["password"].value_or(std::string{}));
        }
        ...
        if (auto rauth = tbl["redis"]["auth"]; rauth)
        {
            ...
            cfg.redis_auth_password = expand_env(rauth["password"].value_or(std::string{}));
        }
        ...
        if (auto rrl = tbl["redis"]["ratelimit"]; rrl)
        {
            ...
            cfg.redis_ratelimit_password = expand_env(rrl["password"].value_or(std::string{}));
        }
