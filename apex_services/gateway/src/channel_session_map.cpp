...
apex::core::Result<void> ChannelSessionMap::subscribe(const std::string& channel, apex::core::SessionId session_id)
{
...
void ChannelSessionMap::unsubscribe(const std::string& channel, apex::core::SessionId session_id)
{
...
const std::vector<apex::core::SessionId>* ChannelSessionMap::get_subscribers(const std::string& channel) const
{
    auto it = channel_to_sessions_.find(channel);
    if (it == channel_to_sessions_.end())
        return nullptr;
    return &it->second;
}

std::vector<std::string> ChannelSessionMap::subscribed_channels() const
{
