...
ConfigReloader::ConfigReloader(
    ...
    RouteUpdateCallback on_route_update,
    RateLimitUpdateCallback on_rate_limit_update)
    : ConfigReloader(io_ctx, std::move(config_path), {},
                     std::move(on_route_update),
                     std::move(on_rate_limit_update)) {}
...
void ConfigReloader::on_file_changed(const std::string& path) {
    ...
    if (core_io_contexts_.empty()) {
        ...
        for (auto& core_io : core_io_contexts_) {
            if (on_route_update_) {
                boost::asio::post(core_io.get(), [cb = on_route_update_, t = ptr]() {
                    cb(t);
                });
            }
            if (on_rate_limit_update_) {
                boost::asio::post(core_io.get(),
                    [cb = on_rate_limit_update_, rl = rl_config]() {
                        cb(rl);
                    });
            }
        }
