...
struct MockRedisReply {
    enum class Type { kString, kInteger, kArray, kNil, kError };
    Type type{Type::kNil};
...
public:
    ...
    [[nodiscard]] apex::core::Result<MockRedisReply>
    command_sync(std::string_view cmd)
    {
        if (fail_commands_) {
    ...
    void set_fail_commands(bool fail) { fail_commands_ = fail; }
