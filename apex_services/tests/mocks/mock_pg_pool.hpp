...
class MockPgPool
{
  public:
    ...
    [[nodiscard]] apex::core::Result<MockPgResult> query(std::string_view sql)
    {
        return query(sql, {});
    }

    /// Execute parameterized query.
    [[nodiscard]] apex::core::Result<MockPgResult> query(std::string_view sql, std::vector<std::string> params)
    {
        if (fail_queries_)
        {
    ...
    [[nodiscard]] apex::core::Result<int> execute(std::string_view sql)
    {
        return execute(sql, {});
    }

    /// Execute parameterized command.
    [[nodiscard]] apex::core::Result<int> execute(std::string_view sql, std::vector<std::string> params)
    {
        if (fail_queries_)
        {
            return apex::core::error(apex::core::ErrorCode::AdapterError);
    ...
    void set_fail_queries(bool fail)
    {
        fail_queries_ = fail;
    }
