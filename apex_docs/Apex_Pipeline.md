...
- Gateway가 자체 프레임워크(BoostAsioCore)로 클라이언트와 직접 통신
- 서비스 간 통신은 전부 Kafka 중앙 버스를 경유
- 즉시 응답이 필요한 경우: Gateway가 로컬에서 처리 (인증 검증 등)
```

> **설계 변경 근거**: gRPC 제거. 자체 프레임워크(BoostAsioCore)가 모든 네트워킹을 담당하며, 서비스 간 통신은 Kafka 중앙 버스로 통일. 동기/비동기 분리의 복잡도를 제거하고, Kafka의 내구성 + fan-out 특성을 전 구간에 활용.
...
| 구간 | 프로토콜 | 라이브러리 |
|------|----------|-----------|
| Client ↔ Gateway | WebSocket / TCP Binary | Boost.Asio + Beast (BoostAsioCore) |
...
| 기법 | 적용 위치 | 효과 |
|------|----------|------|
| **io_context-per-core (shared-nothing)** | 전 서비스 | 코어별 독립 이벤트 루프, 락 제거 |
| **Lock-free MPSC Queue** | 코어 간 통신 | 코어당 수신 큐 1개, O(1) enqueue |
| **Slab Memory Pool** | 코어별 독립 | 핫패스 malloc 제거, O(1) 할당 |
| **Zero-copy Ring Buffer** | 수신 버퍼 | memmove 제거, FlatBuffers 직접 접근 |
| **FlatBuffers 직렬화** | 전 구간 | 역직렬화 비용 제로 (zero-copy 읽기) |
| **Connection Pool** | Service → Redis/PostgreSQL | 커넥션 생성 비용 제거 |
| **Kafka Batch Produce** | Gateway → Kafka | 네트워크 라운드트립 최소화 |
| **CPU Affinity + SO_REUSEPORT** | Gateway / Logic Service | 커널 레벨 코어 분배, 캐시 히트율 극대화 |
| **L1 로컬 캐시** | 전 서비스 | 프로세스 내 해시맵(TTL) → Redis 호출도 제거 |
| **코루틴 프레임 풀 할당** | 핫패스 | promise_type operator new → 슬랩 풀 할당 |
| **배치 I/O (writev)** | 전 서비스 | 시스콜 횟수 최소화 |
| **epoll 기본 / io_uring 선택** | I/O 백엔드 | CMake 옵션으로 io_uring 활성화 가능 |
...
| **Graceful Shutdown** | 전 서비스 | SIGTERM → acceptor 중지 → 코어별 drain → Kafka offset 커밋 → 리소스 정리 → 종료 (기본 25초, 설정 가능) |
...
| **Backpressure** | MPSC 큐 | max_capacity 제한, 80% 시 슬로우다운 → 429 응답 |
...
│   └── CMakeLists.txt
├── gateway/                      ← Gateway 서비스 (C++, 독립 Docker 이미지)
├── services/
│   ├── auth-svc/                 ← 인증 서비스 (C++, 독립 Docker 이미지)
│   ├── chat-svc/                 ← 채팅 서비스 (C++, 독립 Docker 이미지)
│   └── log-svc/                  ← 로그 서비스 (C++, 독립 Docker 이미지)
├── shared/
...
| 카테고리 | 기술 | 라이브러리 |
|----------|------|-----------|
| Language | C++23 | MSVC 19.44+ / GCC 13+ / Clang 17+ |
| Networking | Boost.Asio + Beast | Boost 1.82+ |
| Framework | BoostAsioCore | 자체 개발 (ServiceBase CRTP) |
| Serialization | FlatBuffers | flatbuffers (zero-copy) |
| Message Queue | Apache Kafka (KRaft) | librdkafka 2.x |
| Cache | Redis 7+ | redis-plus-plus |
| DBMS | PostgreSQL 16+ | libpq (Asio 어댑터 직접 구현) |
| Container | Docker | multi-stage build |
| Orchestration | Kubernetes | Helm charts |
| Monitoring | Prometheus + Grafana | prometheus-cpp |
| Logging | spdlog + KafkaSink | spdlog + 구조화 JSON |
| Config | TOML | toml++ (header-only) |
| Auth | JWT + Bloom Filter | jwt-cpp |
| Build | CMake 3.20+ + Ninja | vcpkg (선언형 vcpkg.json) |
| Test | Google Test + Benchmark | gtest + gbenchmark |

---

## 9. BoostAsioCore 프레임워크 핵심 설계
...
class EchoService : public ServiceBase<EchoService> {
    ...
    void on_echo(SessionPtr session, uint16_t msg_id, const EchoRequest* req) {
        // FlatBuffers zero-copy 읽기 + 응답 전송
    }
};
...
- [ ] Redis 키 네이밍 규칙
- [ ] PostgreSQL 스키마 설계
- [ ] Dockerfile + docker-compose (프로파일)
- [ ] K8s manifests + Helm
...
- 자체 프레임워크(BoostAsioCore)가 모든 네트워킹을 담당 → gRPC의 역할이 소멸
...
```
[로컬 개발] docker-compose up --profile minimal → Kafka, Redis, PostgreSQL만 기동
[스테이징] minikube 또는 k3s → 경량 K8s로 오케스트레이션 검증
[프로덕션] K8s (클라우드 매니지드: EKS/GKE)
```
- Dockerfile은 **멀티스테이지 빌드**로 이미지 최소화
- Helm Chart로 환경별 설정 분리
- docker-compose 프로파일: minimal(3개) / observability(+ELK,Prometheus,Grafana) / full
...
- **어떻게?** SIGTERM → acceptor 중지 → 코어별 drain → Kafka offset 커밋 → 종료
- **K8s:** `terminationGracePeriodSeconds` 30초 대비 기본 25초 (5초 여유)
...
| 8 | 프레임워크 | 없음 (각 서비스 개별) | **BoostAsioCore** | ServiceBase CRTP, 코드 재사용 극대화 |
