...
| **Graceful Shutdown** | 전 서비스 | SIGTERM → acceptor 중지 → 코어별 세션 close → 세션 drain 폴링 → CoreEngine 정지 → 서비스 정지 → 종료 (타임아웃 미구현, v0.3.0에서 추가 예정) |
...
| Monitoring | Prometheus + Grafana | prometheus-cpp (v0.4.0+) |
| Logging | spdlog + KafkaSink | spdlog + 구조화 JSON |
| Config | TOML | toml++ (header-only) |
| Auth | JWT + Bloom Filter | jwt-cpp (v0.4.0+) |
...
| 4.7 | 완료 | Server-CoreEngine 통합, cross_core_call, Graceful Shutdown → v0.2.4 | 24개 (14 + 5 + 5) |
...
- **어떻게?** SIGTERM → acceptor 중지 → 코어별 세션 close → 세션 drain 폴링 → CoreEngine 정지 → 서비스 정지 → 종료
- **K8s:** `terminationGracePeriodSeconds` 30초 대비 기본 25초 (5초 여유) (v0.3.0에서 구현 예정)
